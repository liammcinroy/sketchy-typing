...
//// Now trying to automate the standard simplicies
...
template <size_t n, size_t __dim>
struct StandardSimplexSet {
  static constexpr size_t _dim = __dim;
};

template <>
struct StandardSimplexSet<0, 0> {
  static constexpr size_t _dim = 0;
  static constexpr size_t _n_primitives = 0;
...
struct StandardSimplexSet<n, 0> {
  ...
  template <_ object>
  struct materialize {
    using type = FormedVertex<
        object,
        typename StandardSimplexSet<0, 0>::template materialize<0>::type>;
  };
};
...
struct StandardSimplex {
  template <size_t i>
  using Set =
      std::conditional_t<0 <= i && i <= n, StandardSimplexSet<n, i>, void>;
};

using testS0T0 = RequiresTruncated<0, StandardSimplex<0>>;
using testS1T0 = RequiresTruncated<0, StandardSimplex<1>>;
